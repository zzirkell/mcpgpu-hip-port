...
int main(){
    ...
    for(uint32_t ind = 0; ind < start_goal_combinations; ind++){
		...
		gpuErrchk(hipMalloc(&d_eePos_traj, h_eePos_traj.size()*sizeof(linsys_t)));
		gpuErrchk(hipMemcpy(d_eePos_traj, h_eePos_traj.data(), h_eePos_traj.size()*sizeof(linsys_t), hipMemcpyHostToDevice));
		
		gpuErrchk(hipMalloc(&d_xu_traj, h_xu_traj.size()*sizeof(linsys_t)));
		gpuErrchk(hipMemcpy(d_xu_traj, h_xu_traj.data(), h_xu_traj.size()*sizeof(linsys_t), hipMemcpyHostToDevice));
