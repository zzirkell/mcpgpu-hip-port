...
// AMD-friendly line-search merit partial kernel.
// Normal kernel, no cooperative launch, no grid-wide sync.
// Grid layout:
//   blockIdx.x = knot
//   blockIdx.y = alpha candidate
template <typename T>
__global__
void ls_gato_compute_merit_partials(uint32_t state_size,
                                    uint32_t control_size,
                                    uint32_t knot_points,
                                    T *d_xs,
                                    T *d_xu,
                                    ...
                                    T *d_merit_temp)
{
    ...
    const uint32_t thread_id = threadIdx.x;
    const uint32_t num_threads = blockDim.x;
    const uint32_t knot = blockIdx.x;
    const uint32_t alpha_multiplier = blockIdx.y;
    ...
    T Jk, ck, pointmerit;
    T alpha = static_cast<T>(-1) / static_cast<T>(1u << alpha_multiplier);
    ...
    if (knot < knot_points - 1) {
        ck = integratorError<T>(
            state_size,
            s_xux_k,
            &s_xux_k[states_s_controls],
            s_temp,
            d_robotModel,
            dt,
            block
        ...
        // Same terminal-knot behavior as the original line-search merit kernel.
        for (int i = threadIdx.x; i < state_size; i += num_threads) {
            s_temp[i] = abs((d_xu[i] + alpha * d_dz[i]) - d_xs[i]);
        }
...
// Reduce partial merit values.
// One block per alpha candidate.
// Input:  d_merit_temp[alpha * knot_points + knot]
// Output: d_merits_out[alpha]
template <typename T>
__global__
void ls_gato_reduce_merit_partials(uint32_t knot_points,
                                   T *d_merit_temp,
                                   T *d_merits_out)
{
    const cooperative_groups::thread_block block = cooperative_groups::this_thread_block();
    const uint32_t thread_id = threadIdx.x;
    const uint32_t alpha_multiplier = blockIdx.x;

    extern __shared__ T s_reduce[];

    T sum = static_cast<T>(0);

    for (uint32_t knot = thread_id; knot < knot_points; knot += blockDim.x) {
        sum += d_merit_temp[alpha_multiplier * knot_points + knot];
    }

    s_reduce[thread_id] = sum;
    block.sync();

    glass::reduce<T>(blockDim.x, s_reduce);
    block.sync();

    if (thread_id == 0) {
        d_merits_out[alpha_multiplier] = s_reduce[0];
    }
}
