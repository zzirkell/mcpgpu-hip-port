...
T compute_tracking_error(uint32_t state_size, T *d_xu_goal, T *d_xs){
    ...
    T *d_tracking_error;
    gpuErrchk(hipMalloc(&d_tracking_error, sizeof(T)));
    gpuErrchk(hipMemcpy(d_tracking_error, &h_tracking_error, sizeof(T), hipMemcpyHostToDevice));

    compute_tracking_error_kernel<T><<<1,32>>>(d_tracking_error, state_size, d_xu_goal, d_xs);

    gpuErrchk(hipMemcpy(&h_tracking_error, d_tracking_error, sizeof(T), hipMemcpyDeviceToHost));
    gpuErrchk(hipFree(d_tracking_error));
...
// For potentially recording fine-grained timing
    ...
    gpuErrchk(hipMalloc(&d_lambda, state_size*knot_points*sizeof(T)));
    gpuErrchk(hipMalloc(&d_xu, traj_len*sizeof(T)));
    gpuErrchk(hipMalloc(&d_xu_old, traj_len*sizeof(T)));
    gpuErrchk(hipMalloc(&d_eePos_goal, 6*knot_points*sizeof(T)));
    gpuErrchk(hipMemset(d_lambda, 0, state_size*knot_points*sizeof(T)));
    gpuErrchk(hipMemcpy(d_eePos_goal, d_eePos_traj, 6*knot_points*sizeof(T), hipMemcpyDeviceToDevice));
    gpuErrchk(hipMemcpy(d_xu_old, d_xu_traj, traj_len*sizeof(T), hipMemcpyDeviceToDevice));
    ...
    // temp device memory
    T *d_eePos;
    gpuErrchk(hipMalloc(&d_eePos, 6*sizeof(T)));
...
for(int j = 0; j < 100; j++){
    ...
    for(control_update_step = 0; control_update_step < max_control_updates; control_update_step++){
        ...
        gpuErrchk(hipMemcpy(h_eePos, d_eePos, 6*sizeof(T), hipMemcpyDeviceToHost));
    ...
    simulation_time = sqp_solve_time_us;
        ...
        // old xu = new xu
        gpuErrchk(hipMemcpy(d_xu_old, d_xu, traj_len*sizeof(T), hipMemcpyDeviceToDevice));
        ...
        if (!shifted && time_since_timestep > shift_threshold){
            ...
            grid::end_effector_positions_kernel<T><<<1,128,144*sizeof(T)>>>(d_eePos, d_xs, grid::NUM_JOINTS, (grid::robotModel<T> *) d_dynmem, 1);
            gpuErrchk(hipMemcpy(h_eePos, d_eePos, 6*sizeof(T), hipMemcpyDeviceToHost));
            gpuErrchk(hipMemcpy(h_eePos_goal, d_eePos_goal, 6*sizeof(T), hipMemcpyDeviceToHost));
            ...
            if (traj_offset + knot_points < traj_steps){
                ...
                gpuErrchk(hipMemcpy(&d_xu[traj_len - (state_size + control_size)], &d_xu_traj[(state_size+control_size)*traj_offset - control_size], sizeof(T)*(state_size+control_size), hipMemcpyDeviceToDevice));     // last state filled from precomputed trajectory
            }
            else{
                // fill in last state with goal position, zero velocity, last control with zero control
                gpuErrchk(hipMemcpy(&d_xu[traj_len - state_size], &d_xu_traj[(traj_steps-1)*(state_size+control_size)], (state_size/2)*sizeof(T), hipMemcpyDeviceToDevice));
                gpuErrchk(hipMemset(&d_xu[traj_len - state_size / 2], 0, (state_size/2) * sizeof(T)));
                gpuErrchk(hipMemset(&d_xu[traj_len - (state_size+control_size)], 0, control_size * sizeof(T)));
            }
            
            // shift goal
            just_shift(6, 0, knot_points, d_eePos_goal);
            if (traj_offset + knot_points < traj_steps){
                gpuErrchk(hipMemcpy(&d_eePos_goal[(knot_points-1)*(6)], &d_eePos_traj[(traj_offset+knot_points-1) * (6)], 6*sizeof(T), hipMemcpyDeviceToDevice));
            }
            else{
                // fill in last goal state with goal state and zero velocity
                gpuErrchk(hipMemcpy(&d_eePos_goal[(knot_points-1)*(6)], &d_eePos_traj[(traj_steps-1)*(6)], (6)*sizeof(T), hipMemcpyDeviceToDevice));
                // gpuErrchk(hipMemset(&d_eePos_goal[(knot_points-1)*(6) + state_size / 2], 0, (state_size/2) * sizeof(T)));
            }
            
            // shift lambda
            just_shift(state_size, 0, knot_points, d_lambda);
                // gpuErrchk(hipMemset(&lambdas[i][state_size*(knot_points-1)], 0, state_size*sizeof(T)));
        ...
        gpuErrchk(hipMemcpy(d_xu, d_xs, state_size*sizeof(T), hipMemcpyDeviceToDevice));
    ...
    gpuErrchk(hipFree(d_eePos_goal));
    gpuErrchk(hipFree(d_xu_old));

    gpuErrchk(hipFree(d_eePos));
