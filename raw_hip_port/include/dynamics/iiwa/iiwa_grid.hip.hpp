...
namespace grid {
    ...
    T* init_XImats() {
        ...
        T *d_XImats; gpuErrchk(hipMalloc((void**)&d_XImats,504*sizeof(T)));
        gpuErrchk(hipMemcpy(d_XImats,h_XImats,504*sizeof(T),hipMemcpyHostToDevice));
        free(h_XImats);
        return d_XImats;
    }
    ...
    void free_robotModel(robotModel<T>* d_robotModel) {
        robotModel<T> h_robotModel;
        gpuErrchk(hipMemcpy(&h_robotModel,d_robotModel,sizeof(robotModel<T>),hipMemcpyDeviceToHost));
        gpuErrchk(hipFree(h_robotModel.d_XImats));
        gpuErrchk(hipFree(h_robotModel.d_topology_helpers));
