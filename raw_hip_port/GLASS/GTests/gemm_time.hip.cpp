...
int main(int argc, char *argv[]) {
	...
	gpuErrchk(hipMalloc(&d_m, m*n * sizeof(float)));
	gpuErrchk(hipMalloc(&d_v, n * sizeof(float)));
	gpuErrchk(hipMalloc(&d_res, m * sizeof(float)));
	gpuErrchk(hipMemcpy(d_m, h_m, m*n * sizeof(float), hipMemcpyHostToDevice));
	gpuErrchk(hipMemcpy(d_v, h_v, n * sizeof(float), hipMemcpyHostToDevice));
	...
	// both in shared
	hipFuncSetAttribute(reinterpret_cast<const void*>(global_gemv_bothshared), hipFuncAttributeMaxDynamicSharedMemorySize, 99000);
	clock_gettime(CLOCK_MONOTONIC, &start);
	...
	hipFree(d_m); hipFree(d_v); hipFree(d_res);
	free(h_m); free(h_v);
