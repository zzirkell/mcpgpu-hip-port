...
int run_gemv_test(const char* name, T tolerance)
{
    ...
    CHECK_GPU(cudaMalloc((void**)&d_A, matrix_bytes));
    CHECK_GPU(cudaMalloc((void**)&d_x, x_bytes));
    CHECK_GPU(cudaMalloc((void**)&d_y, y_bytes));

    CHECK_GPU(cudaMemcpy(d_A, h_A.data(), matrix_bytes, cudaMemcpyHostToDevice));
    CHECK_GPU(cudaMemcpy(d_x, h_x.data(), x_bytes, cudaMemcpyHostToDevice));
    ...
    CHECK_GPU(cudaMemcpy(h_y.data(), d_y, y_bytes, cudaMemcpyDeviceToHost));
