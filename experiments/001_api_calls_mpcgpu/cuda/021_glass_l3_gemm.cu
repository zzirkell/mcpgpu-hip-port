...
bool run_one_gemm_test(const char* type_name, double tolerance)
{
    ...
    CHECK_GPU(cudaMalloc((void**)&d_a, size_a * sizeof(T)));
    CHECK_GPU(cudaMalloc((void**)&d_b, size_b * sizeof(T)));
    CHECK_GPU(cudaMalloc((void**)&d_c, size_c * sizeof(T)));

    CHECK_GPU(cudaMemcpy(d_a, h_a.data(), size_a * sizeof(T), cudaMemcpyHostToDevice));
    CHECK_GPU(cudaMemcpy(d_b, h_b.data(), size_b * sizeof(T), cudaMemcpyHostToDevice));
    CHECK_GPU(cudaMemcpy(d_c, h_c.data(), size_c * sizeof(T), cudaMemcpyHostToDevice));
    ...
    CHECK_GPU(cudaMemcpy(h_c.data(), d_c, size_c * sizeof(T), cudaMemcpyDeviceToHost));

    CHECK_GPU(cudaFree(d_a));
    CHECK_GPU(cudaFree(d_b));
    CHECK_GPU(cudaFree(d_c));
