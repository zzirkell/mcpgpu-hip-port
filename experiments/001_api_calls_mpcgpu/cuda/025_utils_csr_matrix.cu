...
int main() {
    ...
    CHECK_GPU(cudaMalloc(&d_col_ptr, 1024 * sizeof(QDLDL_int)));
    CHECK_GPU(cudaMalloc(&d_row_ind, 1024 * sizeof(QDLDL_int)));
    CHECK_GPU(cudaMalloc(&d_val, 1024 * sizeof(QDLDL_float)));
    CHECK_GPU(cudaMalloc(&d_src, 1024 * sizeof(float)));
    ...
    CHECK_GPU(cudaMallocPitch(&d_matrix, &pitch, 4 * sizeof(float), 4));
    CHECK_GPU(cudaMemcpy2D(d_matrix, pitch, h_matrix, 4 * sizeof(float), 4 * sizeof(float), 4, cudaMemcpyHostToDevice));

    CHECK_GPU(cudaDeviceSynchronize());

    CHECK_GPU(cudaFree(d_col_ptr));
    CHECK_GPU(cudaFree(d_row_ind));
    CHECK_GPU(cudaFree(d_val));
    CHECK_GPU(cudaFree(d_src));
    CHECK_GPU(cudaFree(d_matrix));
