...
int main() {
    ...
    cudaError_t coop_err = cudaDeviceGetAttribute(
    ...
    CHECK_GPU(cudaOccupancyMaxActiveBlocksPerMultiprocessor(
    ...
    if (max_threads_per_block_attr <= 0) {
        std::fprintf(stderr, "Invalid cudaDevAttrMaxThreadsPerBlock\n");
        return 1;
    }

    if (multiprocessor_count_attr <= 0) {
        std::fprintf(stderr, "Invalid cudaDevAttrMultiProcessorCount\n");
        return 1;
    }
