...
int run_one_type(const char *type_name)
{
    ...
    CHECK_GPU(hipMalloc((void **)&d_x, n * sizeof(T)));
    CHECK_GPU(hipMalloc((void **)&d_y, n * sizeof(T)));
    CHECK_GPU(hipMalloc((void **)&d_out, 3 * sizeof(T)));

    CHECK_GPU(hipMemcpy(d_x, h_x.data(), n * sizeof(T), hipMemcpyHostToDevice));
    CHECK_GPU(hipMemcpy(d_y, h_y.data(), n * sizeof(T), hipMemcpyHostToDevice));
    ...
    CHECK_GPU(hipMemcpy(h_out.data(), d_out, 3 * sizeof(T), hipMemcpyDeviceToHost));
