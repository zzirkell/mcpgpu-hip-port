...
int main(int argc, char** argv) {
    ...
    CHECK_GPU(cudaMalloc(&d_idata, N * sizeof(float)));
    CHECK_GPU(cudaMalloc(&d_odata, sizeof(float)));

    CHECK_GPU(cudaMemcpy(d_idata, h_idata.data(), N * sizeof(float), cudaMemcpyHostToDevice));
    CHECK_GPU(cudaMemcpy(d_odata, &h_odata, sizeof(float), cudaMemcpyHostToDevice));
    ...
    CHECK_GPU(cudaMemcpy(&h_odata, d_odata, sizeof(float), cudaMemcpyDeviceToHost));

    std::cout << "Parallel reduction" << std::endl;
    std::cout << "Expected number:   " << N << std::endl;
    std::cout << "Calculated sum:  " << (int)h_odata << std::endl;
    
    CHECK_GPU(cudaFree(d_idata));
    CHECK_GPU(cudaFree(d_odata));
