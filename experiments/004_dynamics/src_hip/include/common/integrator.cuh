...
void integrator_host(uint32_t state_size, uint32_t control_size, T *d_xs, T *d_xu, void *d_dynMem_const, T dt){
    // T *d_xu;
    // T *d_xs_new;
    // gpuErrchk(hipMalloc(&d_xu, xu_size));
    // gpuErrchk(hipMalloc(&d_xs_new, xs_size));

    // gpuErrchk(hipMemcpy(d_xu, h_xs, state_size*sizeof(T), hipMemcpyHostToDevice));
    // gpuErrchk(hipMemcpy(d_xu + state_size, h_xu + state_size, control_size*sizeof(T), hipMemcpyHostToDevice));
    //TODO: needs sync?
    ...
    //TODO: needs sync?
    // gpuErrchk(hipMemcpy(h_xs, d_xs_new, xs_size, hipMemcpyDeviceToHost));

    // gpuErrchk(hipFree(d_xu));
    // gpuErrchk(hipFree(d_xs_new));
}
...
void just_shift(uint32_t state_size, uint32_t control_size, uint32_t knot_points, T *d_xu){
    for (uint32_t knot = 0; knot < knot_points-1; knot++){
        ...
        gpuErrchk(hipMemcpy(&d_xu[knot*(state_size+control_size)], &d_xu[(knot+1)*(state_size+control_size)], stepsize*sizeof(T), hipMemcpyDeviceToDevice));
    }
}
