...
int main(int argc, char** argv){
    ...
    for(uint32_t ind = 0; ind < start_goal_combinations; ind++){
        ...
        float custom_tol = 0.0f;

        if (argc > 1) {
            custom_tol = static_cast<float>(std::atof(argv[1]));
            use_custom_tol = true;
        ...
        for (uint32_t pcg_exit_ind = pcg_exit_begin;
            ...
            float pcg_exit_tol = use_custom_tol ? custom_tol : pcg_exit_vals[pcg_exit_ind];
		...
		if (USE_HYBRID) {
			test_output_prefix = output_directory_path + std::to_string(KNOT_POINTS) + "_" + "HYBRID" + "_" + std::to_string(pcg_exit_tol);
		} else {
			test_output_prefix = output_directory_path + std::to_string(KNOT_POINTS) + "_" + ( (LINSYS_SOLVE == 1) ? "PCG" : "QDLDL") + "_" + std::to_string(pcg_exit_tol);
		}
