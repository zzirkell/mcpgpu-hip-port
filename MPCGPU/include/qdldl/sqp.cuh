...
void qdldl_solve_schur(const QDLDL_int An,
                       ...
                       volatile int* winner_flag = nullptr){
	...
	QDLDL_factor(An,Ap,Ai,Ax,Lp,Li,Lx,D,Dinv,Lnz,etree,bwork,iwork,fwork, winner_flag);
...
template <typename T>
auto sqpSolveQdldl(uint32_t state_size, uint32_t control_size, uint32_t knot_points, float timestep, T *d_eePos_traj, T *d_lambda, T *d_xu, void *d_dynMem_const, T &rho, T rho_reset, volatile int* winner_flag = nullptr){
    ...
    auto sqpTimecheck = [&]() {
        // has pcg won?
        if (winner_flag != nullptr && *winner_flag != 0) {
    ...
    */
    ...
    for(uint32_t sqpiter = 0; sqpiter < SQP_MAX_ITER; sqpiter++){
        ...
        qdldl_solve_schur(An, h_col_ptr, h_row_ind, h_val, h_gamma, h_lambda, Lp, Li, Lx, D, Dinv, Lnz, etree, bwork, iwork, fwork, winner_flag);
    ...
    return std::make_tuple(linsys_iter_vec, linsys_time_vec, sqp_solve_time, sqp_iter, sqp_time_exit, linsys_exit_vec, 
                           ktt_time_vec, shur_time_vec, dz_time_vec, line_search_time_vec);
}
