...
// For potentially recording fine-grained timing
    ...
    std::tuple<
        ...
        bool,
        std::vector<bool>,
        std::vector<double>, // <-- (ktt)
        std::vector<double>, // <-- (schur)
        std::vector<double>, // <-- (dz)
        std::vector<double>  // <-- (line_search)
    > sqp_stats;
    ...
    gpuErrchk(cudaMalloc(&d_xu_qdldl, traj_len*sizeof(T)));
    
    // winnerflag for both
    volatile int* d_winner_flag;
    gpuErrchk(cudaMallocManaged(&d_winner_flag, sizeof(int)));
    #endif
    ...
    for(control_update_step = 0; control_update_step < max_control_updates; control_update_step++){
        ...
        *d_winner_flag = 0; // 0 = runs, 1 = QDLDL , 2 = PCG 
        ...
        gpuErrchk(cudaMemcpy(d_xu_qdldl, d_xu, traj_len*sizeof(T), cudaMemcpyDeviceToDevice));

        decltype(sqp_stats) stats_qdldl;
        decltype(sqp_stats) stats_pcg;

        std::thread t_qdldl([&]() {
            stats_qdldl = sqpSolveQdldl<T>(state_size, control_size, knot_points, timestep, d_eePos_goal, d_lambda_qdldl, d_xu_qdldl, d_dynmem, rho, rho_reset, d_winner_flag);
            
            if (*d_winner_flag == 0) {
                __sync_val_compare_and_swap(d_winner_flag, 0, 1);
            }
        });
        ...
        if (*d_winner_flag == 1) {
            // qdldl was faster
            hybrid_qdldl_wins++;
            sqp_stats = stats_qdldl;
            gpuErrchk(cudaMemcpy(d_lambda, d_lambda_qdldl, state_size*knot_points*sizeof(T), cudaMemcpyDeviceToDevice));
            gpuErrchk(cudaMemcpy(d_xu, d_xu_qdldl, traj_len*sizeof(T), cudaMemcpyDeviceToDevice));
        } else if (*d_winner_flag == 2) {
            // pcg was faster
            hybrid_pcg_wins++;
            sqp_stats = stats_pcg;
        } else {
